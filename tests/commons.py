...
from zjb._traits.types import TraitAny
...
def deepcopy(obj):
    """深拷贝所有特征"""
    return copy.deepcopy(obj, memo={"traits_copy_mode": "deep"})
...
class DictDataManager(DataManager):
    ...
    def _delete(self, gid: ULID):
        ...
        keys = [key for key in self.dict.keys() if key[:16] == key_prefix]
        for key in keys:
            del self.dict[key]
...
class _TestData(Data):
    ...
    def _trait_added_changed(self, name: str):
        super()._trait_added_changed(name)
        if name.startswith("test"):
            self.store_traits.add(name)
...
traits_parametrize = mark.parametrize(
    "traits",
    [
        {
            "test_none": None,
...
trait_parametrize = mark.parametrize(
    "trait",
    [
        ("test_none", None),
        ("test_int", 1111),
        ...
        ("test_compound_tuple", (1, "a", (1, 2), [1, 2], {1, 2}, {"a": 1})),
        ("test_compound_list", [1, "a", (1, 2), [1, 2], {1, 2}, {"a": 1}]),
        (
            "test_compound_dict",
            {
                "int": 1,
        ...
        ("test_data_in_dict", {"data": _TestData(test_str="Nature", test_int=1111)}),
        ("test_data_in_dict_key", {_TestData(test_str="Nature", test_int=1111): 1}),
        ("test_data_in_set", {_TestData(test_str="Nature", test_int=1111)}),
        (
            "test_data_in_frozenset",
            frozenset([_TestData(test_str="Nature", test_int=1111)]),
        ),
        (
            "test_data_in_data",
            _TestData(test_data=_TestData(test_str="Nature", test_int=1111)),
        ),
        ("test_self_reference_data", create_self_reference_data()),
        ("test_circular_reference_data", create_circular_reference_data()),
        ("test_circular_reference_data_in_list", [create_circular_reference_data()]),
    ],
)

performance_parametrize = mark.parametrize(
    "trait",
    [
        ("test_base", "123"),
    ],
